...
ADCollSensorSplit.LOAD_TIP_COLLISION_APPROACH_DISTANCE = 30
...
function ADCollSensorSplit:shouldIgnoreUnloadTipCollision(transformId, collisionObject)
    if transformId == nil or transformId == 0 or getName(transformId) ~= "tipCollision" then
        return false
    end
    if collisionObject == nil or collisionObject.typeName ~= "productionPoint" then
        return false
    end

    local taskModule = self.vehicle and self.vehicle.ad and self.vehicle.ad.taskModule or nil
    local activeTask = taskModule and taskModule.getActiveTask and taskModule:getActiveTask() or nil
    local isLoadUnloadTask = activeTask ~= nil and (
        activeTask.taskType == "UnloadAtDestinationTask" or
        activeTask.taskType == "LoadAtDestinationTask"
    )
    if not isLoadUnloadTask then
        return false
    end

    if AutoDrive.isInRangeToLoadUnloadTarget(self.vehicle) then
        return true
    end
    if activeTask.taskType ~= "LoadAtDestinationTask" then
        return false
    end

    return AutoDrive.getDistanceToTargetPosition(self.vehicle) <= ADCollSensorSplit.LOAD_TIP_COLLISION_APPROACH_DISTANCE
end
...
function ADCollSensorSplit:collisionTestCallbackSplit(transformId)
    ...
    if self:shouldIgnoreUnloadTipCollision(transformId, collisionObject) then
        return
    end
