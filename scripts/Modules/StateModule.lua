...
ADStateModule.HIGHEST_MODE = AutoDrive.MODE_SNOWPLOW
ADStateModule.SKIPPED_SELECTABLE_MODES = {
    [AutoDrive.MODE_BGA] = true
}
...
function ADStateModule:reset()
    ...
    self.snowPlowMarkers = {}
    self.currentSnowPlowMarkerSlot = nil
...
function ADStateModule:readFromXMLFile(xmlFile, key)
    ...
    local snowPlowMarkers = xmlFile:getValue(key .. "#snowPlowMarkers")
    if snowPlowMarkers ~= nil then
        local markerIds = AutoDrive.stringToNumberList(snowPlowMarkers)
        for slot = 2, 3 do
            local markerId = markerIds[slot - 1]
            if markerId ~= nil and markerId > 0 then
                self.snowPlowMarkers[slot] = ADGraphManager:getMapMarkerById(markerId)
            end
        end
    end
...
function ADStateModule:saveToXMLFile(xmlFile, key)
    ...
    local snowPlowMarkerIds = {}
    for slot = 2, 3 do
        local marker = self.snowPlowMarkers[slot]
        table.insert(snowPlowMarkerIds, marker ~= nil and marker.markerIndex or 0)
    end
    xmlFile:setValue(key .. "#snowPlowMarkers", table.concat(snowPlowMarkerIds, ','))
...
function ADStateModule:doWriteStream(streamId)
    ...
    streamWriteUIntN(streamId, (self.currentSnowPlowMarkerSlot or 0), 2)
...
function ADStateModule:doReadStream(streamId)
    ...
    local currentSnowPlowMarkerSlot = streamReadUIntN(streamId, 2)
    self.currentSnowPlowMarkerSlot = currentSnowPlowMarkerSlot > 0 and currentSnowPlowMarkerSlot or nil
...
function ADStateModule:isSelectableMode(mode)
    if self.vehicle.spec_locomotive and (mode == AutoDrive.MODE_UNLOAD or mode == AutoDrive.MODE_SNOWPLOW) then
        return false
    end

    return mode >= AutoDrive.MODE_DRIVETO
        and mode <= ADStateModule.HIGHEST_MODE
        and ADStateModule.SKIPPED_SELECTABLE_MODES[mode] ~= true
end

function ADStateModule:getNextSelectableMode(mode)
    local nextMode = mode + 1
    while nextMode <= ADStateModule.HIGHEST_MODE do
        if self:isSelectableMode(nextMode) then
            return nextMode
        end
        nextMode = nextMode + 1
    end
...
function ADStateModule:getPreviousSelectableMode(mode)
    local previousMode = mode - 1
    while previousMode >= AutoDrive.MODE_DRIVETO do
        if self:isSelectableMode(previousMode) then
            return previousMode
        end
        previousMode = previousMode - 1
    end

    previousMode = ADStateModule.HIGHEST_MODE
    while previousMode >= AutoDrive.MODE_DRIVETO do
        if self:isSelectableMode(previousMode) then
            return previousMode
        end
...
function ADStateModule:nextMode()
    self.mode = self:getNextSelectableMode(self.mode)
...
function ADStateModule:previousMode()
    self.mode = self:getPreviousSelectableMode(self.mode)
    self:setAutomaticPickupTarget(false) -- disable automatic target on mode change
    self:setAutomaticUnloadTarget(false) -- disable automatic target on mode change
    AutoDrive.Hud.lastUIScale = 0
    self:raiseDirtyFlag()
...
function ADStateModule:setMode(newMode)
    if self:isSelectableMode(newMode) and newMode ~= self.mode then
        self.mode = newMode
        self:setAutomaticPickupTarget(false) -- disable automatic target on mode change
        self:setAutomaticUnloadTarget(false) -- disable automatic target on mode change
        AutoDrive.Hud.lastUIScale = 0
...
function ADStateModule:setSnowPlowMarker(slot, markerId)
    if slot == nil or slot < 2 or slot > 3 then
        return
    end
    ...
    if markerId ~= nil and markerId > 0 then
        ...
        self.snowPlowMarkers[slot] = nil
    end
    self:raiseDirtyFlag()
end

function ADStateModule:setCurrentSnowPlowMarkerSlot(slot)
    if slot ~= nil and (slot < 1 or slot > 3) then
        slot = nil
    end

    if slot ~= self.currentSnowPlowMarkerSlot then
        self.currentSnowPlowMarkerSlot = slot
        self:raiseDirtyFlag()
    end
end

function ADStateModule:getCurrentSnowPlowMarkerSlot()
    return self.currentSnowPlowMarkerSlot
end

function ADStateModule:getSnowPlowMarker(slot)
    if slot == 1 then
        return self:getFirstMarker()
    end

    if self.snowPlowMarkers == nil then
        self.snowPlowMarkers = {}
    end

    return self.snowPlowMarkers[slot]
end

function ADStateModule:getSnowPlowMarkerId(slot)
    local marker = self:getSnowPlowMarker(slot)
    if marker ~= nil then
...
function ADStateModule:getSnowPlowMarkers()
    local markers = {}
    local usedMarkerIds = {}

    for slot = 1, 3 do
        local marker = self:getSnowPlowMarker(slot)
        if marker ~= nil and marker.id ~= nil and usedMarkerIds[marker.markerIndex] ~= true then
            table.insert(markers, marker)
            usedMarkerIds[marker.markerIndex] = true
        end
    end
...
function ADStateModule:setSpeedLimit(speedLimit)
    self.speedLimit = math.min(speedLimit, AutoDrive.getVehicleMaxSpeed(self.vehicle))
    self:raiseDirtyFlag()
end
...
function ADStateModule:setFieldSpeedLimit(speedLimit)
    self.fieldSpeedLimit = math.min(speedLimit, AutoDrive.getVehicleMaxSpeed(self.vehicle))
    self:raiseDirtyFlag()
end
...
function ADStateModule:resetMarkersOnReload()
    ...
    for slot = 2, 3 do
        local oldSnowMarker = self:getSnowPlowMarker(slot)
        local newSnowMarker = nil
        if oldSnowMarker ~= nil and oldSnowMarker.id ~= nil then
            newSnowMarker = ADGraphManager:getMapMarkerByWayPointId(oldSnowMarker.id)
        end
        self.snowPlowMarkers[slot] = newSnowMarker
    end
