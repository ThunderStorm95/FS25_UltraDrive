...
ADDrivePathModule.CP_HANDOFF_DIAGNOSTIC_TIMEOUT = 60000
ADDrivePathModule.CP_HANDOFF_ALIGNMENT_GRACE = 15000
ADDrivePathModule.CP_HANDOFF_ALIGNMENT_MIN_SPEED = 0.0005
ADDrivePathModule.CP_HANDOFF_ALIGNMENT_MIN_STEERING = 0.1
...
function ADDrivePathModule:followWaypoints(dt)
    ...
    local snowPlowSpeedOverride = ADSnowPlowUtil.getSnowPlowSpeedOverride(self.vehicle)
    self.speedLimit = snowPlowSpeedOverride or self.vehicle.ad.stateModule:getSpeedLimit()
    if ((g_updateLoopIndex + self.vehicle.id) % AutoDrive.PERF_FRAMES_HIGH == 0) then
        self.speedLimit = snowPlowSpeedOverride or self.vehicle.ad.stateModule:getSpeedLimit()
        if AutoDrive.checkIsOnField(x, y, z) then
            self.speedLimit = snowPlowSpeedOverride or self.vehicle.ad.stateModule:getFieldSpeedLimit() --math.min(self.vehicle.ad.stateModule:getFieldSpeedLimit(), self.speedLimit)
        end
...
function ADDrivePathModule:isCourseplayHandoffAligning()
    local cpHandoffElapsed = self.vehicle.ad.cpHandoffDiagnosticElapsed or math.huge
    local lastSpeedReal = math.abs(self.vehicle.lastSpeedReal or 0)
    local rotatedTime = math.abs(self.vehicle.rotatedTime or 0)
    return self.vehicle.ad.cpHandoffDiagnosticActive == true
        and self.vehicle.ad.cpHandoffDiagnosticMode == self.vehicle.ad.stateModule:getMode()
        and cpHandoffElapsed <= ADDrivePathModule.CP_HANDOFF_ALIGNMENT_GRACE
        and lastSpeedReal >= ADDrivePathModule.CP_HANDOFF_ALIGNMENT_MIN_SPEED
        and rotatedTime >= ADDrivePathModule.CP_HANDOFF_ALIGNMENT_MIN_STEERING
end

function ADDrivePathModule:checkIfStuck(dt)
    if self.vehicle.isServer then
        if self.vehicle.ad.cpHandoffDiagnosticActive then
            local cpHandoffStartedAt = self.vehicle.ad.cpHandoffDiagnosticStartedAt or g_time
            local cpHandoffElapsed = math.max(0, g_time - cpHandoffStartedAt)
            self.vehicle.ad.cpHandoffDiagnosticElapsed = cpHandoffElapsed
            if cpHandoffElapsed > ADDrivePathModule.CP_HANDOFF_DIAGNOSTIC_TIMEOUT
                or self.vehicle.ad.cpHandoffDiagnosticMode ~= self.vehicle.ad.stateModule:getMode()
            then
                self.vehicle.ad.cpHandoffDiagnosticActive = false
            end
        end

        local wp = self:getCurrentWayPoint()
        local isStoppingVehicle = self.vehicle.ad.specialDrivingModule:isStoppingVehicle()
        if not isStoppingVehicle and wp ~= nil then
            local x, _, z = getWorldTranslation(self.vehicle.components[1].node)
            local distanceToNextWayPoint = MathUtil.vector2Length(x - wp.x, z - wp.z)
            if self:isCourseplayHandoffAligning() then
                self.minDistanceTimer:timer(false)
                self.minDistanceToNextWp = distanceToNextWayPoint
                if not self.vehicle.ad.cpHandoffAlignmentLogged then
                    self.vehicle.ad.cpHandoffAlignmentLogged = true
                    Logging.info(
                        "[UltraDrive][CpHandoffDiag] alignment grace vehicle=%s elapsed=%s lastSpeedReal=%s rotatedTime=%s waypointId=%s waypointIndex=%s currentDistance=%s",
                        tostring(self.vehicle.ad.stateModule:getName()),
                        tostring(self.vehicle.ad.cpHandoffDiagnosticElapsed),
                        tostring(self.vehicle.lastSpeedReal),
                        tostring(self.vehicle.rotatedTime),
                        tostring(wp.id),
                        tostring(self:getCurrentWayPointIndex()),
                        tostring(distanceToNextWayPoint)
                    )
                end
            else
                ...
                if self.minDistanceTimer:done() then
                    ...
                    local taskInfo = activeTask and activeTask.getI18nInfo and activeTask:getI18nInfo() or nil
                    Logging.info(
                        "[UltraDrive][StuckDiag] vehicle=%s mode=%s task=%s waypointId=%s waypointIndex=%s pathLength=%s currentDistance=%s bestDistance=%s timerElapsed=%s lastSpeedReal=%s vehicleX=%s vehicleZ=%s waypointX=%s waypointZ=%s targetX=%s targetZ=%s rotatedTime=%s speedLimit=%s collisionDetected=%s isOnRoadNetwork=%s isStoppingVehicle=%s cpHandoffActive=%s cpHandoffElapsed=%s restartCP=%s isCpEmpty=%s isCpFull=%s",
                        tostring(self.vehicle.ad.stateModule:getName()),
                        tostring(self.vehicle.ad.stateModule:getMode()),
                        tostring(taskInfo),
                        tostring(wp.id),
                        tostring(self:getCurrentWayPointIndex()),
                        tostring(self.wayPoints and #self.wayPoints or 0),
                        tostring(distanceToNextWayPoint),
                        tostring(self.minDistanceToNextWp),
                        tostring(self.minDistanceTimer.elapsedTime),
                        tostring(self.vehicle.lastSpeedReal),
                        tostring(x),
                        tostring(z),
                        tostring(wp.x),
                        ...
                        tostring(self.targetZ),
                        tostring(self.vehicle.rotatedTime),
                        tostring(self.speedLimit),
                        tostring(self.vehicle.ad.collisionDetectionModule.detectedObstable),
                        tostring(self:isOnRoadNetwork()),
                        tostring(isStoppingVehicle),
                        tostring(self.vehicle.ad.cpHandoffDiagnosticActive == true),
                        tostring(self.vehicle.ad.cpHandoffDiagnosticElapsed),
                        tostring(self.vehicle.ad.restartCP),
                        tostring(self.vehicle.ad.isCpEmpty),
                        tostring(self.vehicle.ad.isCpFull)
                    )
