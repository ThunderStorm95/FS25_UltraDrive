...
function ADSnowPlowUtil.accrueServicePay(mode, dt)
    ...
    local wageCost = ADSnowPlowUtil.getServiceDriverWageCost(dt)
    ...
    if mode.snowServiceLastDebugMs >= ADSnowPlowUtil.SERVICE_DEBUG_INTERVAL_MS then
        AutoDrive.debugMsg(
            vehicle,
            "Snow plow service pending: $%.2f elapsed=%.1fs distance=%.1fm wageCost=%.2f fuelCost=%.2f lastTick=%.1fm",
...
function ADSnowPlowUtil.settleServicePay(mode, reason)
    ...
    AutoDrive.debugMsg(
        vehicle,
        "Snow plow service paid: +$%d reason=%s elapsed=%.1fs distance=%.1fm wageCost=%.2f fuelCost=%.2f margin=%.0f%%",
        payAmount,
        tostring(reason),
        (mode.snowServiceElapsedMs or 0) / 1000,
        mode.snowServiceDistance or 0,
