...
AutoDriveHudInputEventEvent.TYPE_SNOW_PLOW_MARKER = 7
...
function AutoDriveHudInputEventEvent.new(vehicle, eventType, value, extraValue)
    local self = AutoDriveHudInputEventEvent.emptyNew()
    self.vehicle = vehicle
    self.eventType = eventType
    self.value = value
    self.extraValue = extraValue or 0
    return self
end
...
function AutoDriveHudInputEventEvent:writeStream(streamId, connection)
    ...
    streamWriteUIntN(streamId, self.value, 20)
    streamWriteUInt8(streamId, self.extraValue or 0)
end
...
function AutoDriveHudInputEventEvent:readStream(streamId, connection)
    ...
    self.value = streamReadUIntN(streamId, 20)
    self.extraValue = streamReadUInt8(streamId)
    self:run(connection)
end
...
function AutoDriveHudInputEventEvent:run(connection)
    if g_server ~= nil then
        ...
        if self.eventType == self.TYPE_SNOW_PLOW_MARKER then
            self.vehicle.ad.stateModule:setSnowPlowMarker(self.extraValue, self.value)
        end
    end
end
...
function AutoDriveHudInputEventEvent:sendSnowPlowMarkerEvent(vehicle, slot, markerId)
    if g_client ~= nil then
        -- Client have to send to server
        g_client:getServerConnection():sendEvent(AutoDriveHudInputEventEvent.new(vehicle, self.TYPE_SNOW_PLOW_MARKER, markerId, slot))
    end
end
