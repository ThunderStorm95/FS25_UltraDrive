...
ADPullDownList.TYPE_SNOW_TARGET_2 = 4
ADPullDownList.TYPE_SNOW_TARGET_3 = 5
...
function ADPullDownList:onDraw(vehicle, uiScale)
    ...
    if self.state == ADPullDownList.STATE_COLLAPSED then
        ...
        if vehicle.ad.stateModule:isActive() then
            local isSnowPlowTargetRow = vehicle.ad.stateModule:getMode() == AutoDrive.MODE_SNOWPLOW and (self.type == ADPullDownList.TYPE_TARGET or self:isSnowTargetType())
            if isSnowPlowTargetRow and vehicle.ad.stateModule:getCurrentSnowPlowMarkerSlot() == self:getSnowTargetSlot() then
                setTextColor(unpack(AutoDrive.currentColors.ad_color_hudTextHover))
            elseif not isSnowPlowTargetRow then
                ...
                elseif self:isSnowTargetType() and vehicle.ad.stateModule:getSnowPlowMarker(self:getSnowTargetSlot()) ~= nil then
                    targetToCheck = vehicle.ad.stateModule:getSnowPlowMarker(self:getSnowTargetSlot()).name
                end
                local actualTarget = ""
...
function ADPullDownList:getVisibleIndexForReturnValue(vehicle, returnValue)
    for index = 1, self:getItemCount() do
        local entry = self:getListElementByIndex(vehicle, index)
        if entry ~= nil and entry.isFolder == false and entry.returnValue == returnValue then
            return index
        end
    end
    return nil
...
function ADPullDownList:getVisibleItemCount(vehicle)
    local count = 0
    for index = 1, self:getItemCount() do
        if self:getListElementByIndex(vehicle, index) ~= nil then
            count = index
        end
    end
    return count
...
function ADPullDownList:openGroupForReturnValue(vehicle, returnValue)
    if self.type == ADPullDownList.TYPE_FILLTYPE or not AutoDrive.getSetting("useFolders") then
        return
    end
    for groupID, entries in pairs(self.options) do
        for _, entry in pairs(entries) do
            if entry.returnValue == returnValue then
...
function ADPullDownList:updateVisibility(vehicle)
    ...
    if self:isSnowTargetType() then
        newVisibility = vehicle.ad.stateModule:getMode() == AutoDrive.MODE_SNOWPLOW
    end
...
function ADPullDownList:createSelection()
    ...
    elseif self.type == ADPullDownList.TYPE_UNLOAD then
        self:createSelection_Target()
        self:sortCurrentItems()
    elseif self:isSnowTargetType() then
        self:createSelection_SnowTarget()
        self:sortCurrentItems()
...
function ADPullDownList:createSelection_SnowTarget()
    self:createSelection_Target()
    if self.options[1] == nil then
        self.options[1] = {}
    end
    table.insert(self.options[1], {displayName = g_i18n:getText("gui_ad_none"), returnValue = 0})
end
...
function ADPullDownList:getNewState(vehicle)
    ...
    elseif self:isSnowTargetType() then
        self:getNewState_SnowTarget(vehicle)
    elseif self.type == ADPullDownList.TYPE_FILLTYPE then
...
function ADPullDownList:getNewState_SnowTarget(vehicle)
    local newState = self.state
    local newSelection = self.selected
    if self.state == ADPullDownList.STATE_COLLAPSED then
        local slot = self:getSnowTargetSlot()
        local markerSelected = vehicle.ad.stateModule:getSnowPlowMarker(slot)
        if markerSelected ~= nil then
            self.text = markerSelected.name
        else
            self.text = string.format("Place %d", slot)
        end
    end
    return newState, newSelection
end
...
function ADPullDownList:expand(vehicle)
    ...
    if itemCount > 0 then
        ...
        local visibleItemCount = self:getVisibleItemCount(vehicle)
        if visibleItemCount == 0 then
            Logging.info("[AutoDrive] PullDownList empty type=%s cached=%s visible=%s filter=%s selected=%s hovered=%s firstMarker=%s secondMarker=%s",
                tostring(self.type), tostring(itemCount), tostring(visibleItemCount), tostring(vehicle.ad.destinationFilterText),
                tostring(self.selected), tostring(self.hovered), tostring(vehicle.ad.stateModule:getFirstMarkerId()),
                tostring(vehicle.ad.stateModule:getSecondMarkerId()))
        end
    end
end
...
function ADPullDownList:collapse(vehicle, setItem)
    ...
    if self.hovered ~= nil and setItem ~= nil and setItem == true then
        ...
        if selectedEntry ~= nil and selectedEntry.returnValue ~= nil and selectedEntry.isFolder == false then
            ...
            elseif self:isSnowTargetType() then
                if selectedEntry.returnValue <= 0 or ADGraphManager:getMapMarkerById(selectedEntry.returnValue) ~= nil then
                    AutoDriveHudInputEventEvent:sendSnowPlowMarkerEvent(vehicle, self:getSnowTargetSlot(), selectedEntry.returnValue)
                end
            elseif self.type == ADPullDownList.TYPE_FILLTYPE then
...
function ADPullDownList:setSelected(vehicle)
    self.selected = 1
    self.hovered = 1
    local selectedReturnValue = nil
    if self.type == ADPullDownList.TYPE_TARGET then
        selectedReturnValue = vehicle.ad.stateModule:getFirstMarkerId()
    elseif self.type == ADPullDownList.TYPE_UNLOAD then
        selectedReturnValue = vehicle.ad.stateModule:getSecondMarkerId()
    elseif self:isSnowTargetType() then
        selectedReturnValue = vehicle.ad.stateModule:getSnowPlowMarkerId(self:getSnowTargetSlot())
    elseif self.type == ADPullDownList.TYPE_FILLTYPE then
        selectedReturnValue = vehicle.ad.stateModule:getFillType()
    end

    if selectedReturnValue ~= nil then
        self:openGroupForReturnValue(vehicle, selectedReturnValue)
        local visibleIndex = self:getVisibleIndexForReturnValue(vehicle, selectedReturnValue)
        if visibleIndex ~= nil then
            self.selected = visibleIndex
            self.hovered = visibleIndex
        end
    end

    local reachedTop = false
    local numberOfElementsVisible = math.min(self:getVisibleItemCount(vehicle), ADPullDownList.MAX_SHOWN - 2)
...
function ADPullDownList:isSnowTargetType()
    return self.type == ADPullDownList.TYPE_SNOW_TARGET_2
        or self.type == ADPullDownList.TYPE_SNOW_TARGET_3
end

function ADPullDownList:getSnowTargetSlot()
    if self.type == ADPullDownList.TYPE_SNOW_TARGET_2 then
        return 2
    elseif self.type == ADPullDownList.TYPE_SNOW_TARGET_3 then
        return 3
    end
    return 1
end
