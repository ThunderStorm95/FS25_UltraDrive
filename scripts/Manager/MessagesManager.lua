...
function ADMessagesManager:loadHud(hud)
    ...
    hud.headerOverlay = Overlay.new(g_baseUIFilename, hud.posX, hud.posY + (textSize * 1.6), 0, hud.headerOverlayHeight)
    hud.headerOverlay:setUVs(g_colorBgUVs)
    hud.headerOverlay:setAlignment(Overlay.ALIGN_VERTICAL_BOTTOM, Overlay.ALIGN_HORIZONTAL_CENTER)
    hud.backgroundOverlay = Overlay.new(g_baseUIFilename, hud.posX, hud.posY, 0, hud.backgroundOverlayHeight)
    hud.backgroundOverlay:setUVs(g_colorBgUVs)
...
function ADMessagesManager:drawHud(hud)
    ...
    hud.backgroundOverlay:setColor(unpack(AutoDrive.currentColors.ad_color_background))
    hud.backgroundOverlay:render()
    hud.headerOverlay:setColor(unpack(AutoDrive.currentColors.ad_color_header))
