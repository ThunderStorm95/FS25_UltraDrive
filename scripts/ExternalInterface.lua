            ...
            if not vehicle.ad.stateModule:isActive() then
                if vehicle.ad.stateModule:getStartHelper() and vehicle.ad.stateModule:getUsedHelper() == ADStateModule.HELPER_CP then
                    ...
                    if table.contains(AutoDrive.modesToStartFromCP, vehicle.ad.stateModule:getMode()) then
                        ...
                        vehicle.ad.cpHandoffDiagnosticElapsed = 0
                        vehicle.ad.cpHandoffDiagnosticStartedAt = g_time
                        vehicle.ad.cpHandoffDiagnosticMode = vehicle.ad.stateModule:getMode()
                        vehicle.ad.cpHandoffDiagnosticActive = true
                        vehicle.ad.cpHandoffAlignmentLogged = false
                        Logging.info(
                            "[UltraDrive][CpHandoffDiag] start vehicle=%s mode=%s restartCP=%s isCpEmpty=%s isCpFull=%s lastSpeedReal=%s distanceFromNetwork=%s",
                            tostring(vehicle.ad.stateModule:getName()),
                            tostring(vehicle.ad.stateModule:getMode()),
                            tostring(vehicle.ad.restartCP),
                            tostring(vehicle.ad.isCpEmpty),
                            tostring(vehicle.ad.isCpFull),
                            tostring(vehicle.lastSpeedReal),
                            tostring(ADGraphManager:getDistanceFromNetwork(vehicle))
                        )
