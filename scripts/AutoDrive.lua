...
function AutoDrive.getModMetadata()
	local modName = AutoDrive.currentModName or g_currentModName
	if g_modManager ~= nil and modName ~= nil and g_modManager.getModByName ~= nil then
		return g_modManager:getModByName(modName)
	end
	return nil
end

function AutoDrive.getDisplayName()
	local mod = AutoDrive.getModMetadata()
	if mod ~= nil and mod.title ~= nil and mod.title ~= "" then
		return mod.title
	end
	return string.gsub(tostring(AutoDrive.currentModName or g_currentModName or "AutoDrive"), "^FS%d+_", "")
end

function AutoDrive.getDisplayVersion()
	local mod = AutoDrive.getModMetadata()
	if mod ~= nil and mod.version ~= nil and mod.version ~= "" then
		return mod.version
	end
	return AutoDrive.version
end

function AutoDrive.getCourseplayInstance()
	local courseplayEnvironment = _G["FS25_Courseplay"]
	if courseplayEnvironment ~= nil and courseplayEnvironment.g_Courseplay ~= nil then
		return courseplayEnvironment.g_Courseplay
	end
	return g_Courseplay
end

AutoDrive.brandTranslationKeys = {
	"gui_ad_Setting",
	"gui_ad_settingsPage_title",
	"gui_ad_userSettingsPage_title",
	"gui_ad_vehicleSettingsPage_title",
	"gui_ad_combineUnloadSettingsPage_title",
	"gui_ad_debugSettingsPage_title",
	"gui_ad_eFeaturesSettingsPage_title",
	"gui_ad_environmentSettingsPage_title"
}

function AutoDrive.updateBrandTranslations(texts)
	if texts == nil then
		return
	end

	local displayName = AutoDrive.getDisplayName()
	if displayName == nil or displayName == "" then
		return
	end

	for _, key in ipairs(AutoDrive.brandTranslationKeys) do
		local text = texts[key]
		if type(text) == "string" then
			text = string.gsub(text, "AutoDrive", displayName)
			text = string.gsub(text, "Autodrive", displayName)
			texts[key] = text
		end
	end
