...
function AutoDrive.getSaveKey(key)
    local adKey = string.gsub(key, "FS25_AutoDrive.AutoDriveVehicleData", "AutoDrive")
    adKey = string.gsub(adKey, "FS25_UltraDrive.AutoDriveVehicleData", "AutoDrive")
    adKey = string.gsub(adKey, "FS25_AutoDrive.AutoDrive", "AutoDrive")
    adKey = string.gsub(adKey, "FS25_UltraDrive.AutoDrive", "AutoDrive")
    return adKey
end
...
function AutoDrive.initSpecialization()
    ...
    schemaSavegame:register(XMLValueType.STRING, "vehicles.vehicle(?).AutoDrive#snowPlowMarkers", "snowPlowMarkers")
...
function AutoDrive:onLoad(savegame)
    ...
    self.ad.modes[AutoDrive.MODE_SNOWPLOW] = SnowPlowMode:new(self)
...
function AutoDrive:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if self.ad ~= nil and self.ad.stateModule ~= nil and self.ad.stateModule:getMode() == AutoDrive.MODE_SNOWPLOW then
        ADSnowPlowUtil.monitorSnowToolState(self, dt)
    end
...
function AutoDrive:saveToXMLFile(xmlFile, key, usedModNames)
    ...
    local adKey = AutoDrive.getSaveKey(key)
...
function AutoDrive:stopAutoDrive()
    ...
    if self.isServer then
        ...
        self.ad.cpHandoffDiagnosticActive = false
        self.ad.cpHandoffDiagnosticElapsed = nil
        self.ad.cpHandoffDiagnosticStartedAt = nil
        self.ad.cpHandoffDiagnosticMode = nil
        self.ad.cpHandoffAlignmentLogged = false
