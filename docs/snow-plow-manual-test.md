...
This is a reusable per-run worksheet, not an implementation backlog. Unchecked boxes mean "not exercised in the current test run," not "feature work remains incomplete." Start with a fresh copy or reset the boxes for each regression session.
...
- [ ] Three-attempt policy counts completed physical-stuck recovery maneuvers followed by renewed stalls: after three `SUCCEEDED` maneuvers each followed by another detected stall, AutoDrive stops on the next detection instead of starting a fourth maneuver.
- [ ] A sensor-held stop at a long train or fixed structure does not start recovery. After 15 seconds it shows one blocked-route warning and continues waiting.
- [ ] When the train clears, driving resumes and a later obstruction can produce one new warning.
- [ ] For an obstruction-classification reproduction, enumerate `Used Start Parameters` markers chronologically, isolate the actual tested-run segment from its marker through the line before the next marker, and separately review any later menu/soft-restart segment. Do not let the newest marker omit tested evidence. Record waypoint, sensor state, obstruction duration, recovery-start count, route target, and payment settlement from the tested-run segment.
- [ ] Do not broaden physical recovery for a sensor-held stop unless the UltraDrive change includes both a tested obstruction classifier and a rollback package for the exact candidate.
- [ ] If the tractor gets sideways on a bridge while driving to the first selected marker, it does not spin in place for more than one recovery cycle.
  - 2026-07-22 evidence: the [Field 110 recovery pilot](investigations/2026-07-22-field110-recovery-pilot.md) did not produce a comparable bridge pass. Two Snow Plow start invocations instead exposed `nil` downstream route data in the active task after a valid 703-waypoint build; a later recovery attempt timed out in `verifyInitiallyLowered` and stopped under control. The bridge outcome remains inconclusive.
...
- [ ] A normal pause of up to five seconds still accrues service wages; a longer stationary pause stops wage accrual until movement or recovery resumes.
- [ ] Actual tracked fuel cost remains reimbursable while wage accrual is paused.
- [ ] Debug payment lines distinguish `elapsed`, `productive`, and current `idle` time and include `wageCost`, `fuelCost`, and `margin=15%`.
...
SnowPlowLoopTask: persistent obstacle; waiting speed=... waypoint=...
Snow plow service pending: $... elapsed=... productive=... idle=... distance=... wageCost=... fuelCost=...
Snow plow service paid: +$... reason=... elapsed=... productive=... idle=... distance=... wageCost=... fuelCost=... margin=15%
```
...
- For an obstruction-classification reproduction, the sensor state, obstruction duration, recovery-start count, route target, and payment settlement.
...
- [ ] In `log.txt`, enumerate `Used Start Parameters` markers chronologically, isolate the actual tested-run segment from its marker through the line before the next marker, and separately review any later menu/soft-restart segment for `RecoveryManeuver`, `SnowPlow`, `Error:`, `Warning:`, and `Lua call stack`. Do not let the newest marker omit tested evidence.
...
## Optional Marker Clearing

- [ ] In Snow Plow mode, open Place 2 and select the localized `None` entry; the row returns to `Place 2`.
- [ ] Repeat for Place 3; the row returns to `Place 3`.
- [ ] Dealer -> Refuel -> Dealer works with Place 3 cleared instead of duplicating Dealer.
- [ ] Clearing Place 2 or Place 3 survives save/reload without a Lua or multiplayer stream error.

## Waypoint Drag Diagnostic

The right/middle-button movement handlers remain the unchanged upstream implementation. Do not modify them unless this controlled check reproduces a defect.

2026-07-23 acceptance note: the corrected build resolves Courseplay through its `FS25_Courseplay` mod environment and late-binds UltraDrive during `loadMap`. In a no-save savegame 5 run on the John Deere 6R 250, the log confirmed `bound=true`; with edit mode active, bounded right-clicks changed UltraDrive's editor state while Courseplay stayed minimized. The final ZIP passed 115 tests and has SHA-256 `91BCF0E7B32DC6B4298CDAD272C703C66F5DF83E7CB6EF89110C0F7874B2D4D6`. The user subsequently confirmed the right- and middle-button waypoint movement worked correctly in gameplay.

- [x] Stop AutoDrive and close all non-editor dialogs.
- [x] On the test vehicle, enable Courseplay's `Open HUD with mouse` setting so the compatibility path is exercised.
- [x] Enable extended editor mode and ensure the AutoDrive mouse cursor is active.
- [x] Hover a visible waypoint until it is highlighted.
- [x] With Shift, Ctrl, Alt, and right Shift released, hold the middle mouse button and drag the waypoint.
- [x] Repeat with the right mouse button.
- [x] Verify the waypoint moves while held and remains at the new position on release.
- [x] Verify Courseplay's right-button HUD/cursor behavior still works when the UltraDrive editor cursor is not active.
- [ ] If either attempt fails, capture the Courseplay mouse setting, editor mode, active GUI, modifier-key state, `hoveredNodeId`, and `nodeToMoveId` before changing code.
