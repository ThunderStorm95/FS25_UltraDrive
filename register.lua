...
source(Utils.getFilename("scripts/Utils/CircularRouteUtil.lua", g_currentModDirectory))
source(Utils.getFilename("scripts/Utils/RecoveryManeuver.lua", g_currentModDirectory))
source(Utils.getFilename("scripts/Utils/SnowPlowUtil.lua", g_currentModDirectory))
...
source(Utils.getFilename("scripts/Tasks/SnowPlowLoopTask.lua", g_currentModDirectory))
source(Utils.getFilename("scripts/Tasks/SnowPlowDriveToStartTask.lua", g_currentModDirectory))
...
source(Utils.getFilename("scripts/Modes/SnowPlowMode.lua", g_currentModDirectory))
...
-- Preserve integrations that resolve AutoDrive through the original mod environment name.
if g_currentModName == "FS25_UltraDrive" and _G["FS25_AutoDrive"] == nil then
    _G["FS25_AutoDrive"] = _G[g_currentModName]
end
...
function AutoDriveRegister:loadMap(name)
	...
	local courseplay = AutoDrive.getCourseplayInstance()
	local courseplayPreviouslyBound = courseplay ~= nil and courseplay.autoDrive ~= nil
	if courseplay ~= nil and courseplay.autoDrive == nil then
		courseplay.autoDrive = AutoDrive
	end
	Logging.info(
		"[AutoDrive] Courseplay compatibility binding found=%s previouslyBound=%s bound=%s",
		tostring(courseplay ~= nil),
		tostring(courseplayPreviouslyBound),
		tostring(courseplay ~= nil and courseplay.autoDrive == AutoDrive)
	)
end
...
function AutoDriveRegister.addModTranslations(i18n)
	...
	AutoDrive.updateBrandTranslations(global)
end
...
local function check()
    ...
    if mods then
        for _, activemod in pairs(mods) do
            if activemod.title == "UltraDrive" or activemod.title == "AutoDrive" then
                foundMod = true
                break
            end
        end
    ...
    if g_currentModName then
        if (g_currentModName == "FS25_UltraDrive" or g_currentModName == "FS25_AutoDrive") and foundMod then
            ...
            local args = {text = "UltraDrive: Wrong mod file name / Falscher Dateiname " .. g_currentModName .. " <--> FS25_UltraDrive or FS25_AutoDrive or mod corrupt / oder Mod defekt"}
